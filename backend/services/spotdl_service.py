...
class SpotDLService:
    ...
    async def download_playlist(self, url: str, job_id: Optional[str] = None):
        if not self.validate_spotify_url(url):
            yield DownloadResult(job_id=job_id or str(uuid.uuid4()), success=False, output_dir=Path(""), song_count=0, error="Invalid Spotify URL")
            return
        ...
        try:
            ...
            async def read_output(stream, name):
                while True:
                    ...
                    if text: logger.info(f"[spotdl {name}] {text}")
            
            await asyncio.gather(read_output(process.stdout, "stdout"), read_output(process.stderr, "stderr"))
            await process.wait()
